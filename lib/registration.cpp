                        ...
                        EigenNormalSet normals)
{
  ...
  for (; lambda < LMBD_MAX; lambda *= 2.0) {
    for (auto i : temporary_supervoxels) {
      ...
      while (!point_queue.empty()) {
        ...
        double loss =
          sizes[current] *
          point_2_point_VCCS_distance(epoch.cloud.row(i),epoch.cloud.row(current),
  ...
  for (size_t i = 0; i < epoch.cloud.rows(); ++i) {
    labels[i] = set.Find(i);
    distances.push_back(
      point_2_point_VCCS_distance(epoch.cloud.row(i),epoch.cloud.row(labels[i]),
                          normals.row(i), normals.row(labels[labels[i]]), resolution);
  }
  ...
  while (!boundaries_queue.empty()) {
    ...
    for (auto j : result[current_point].first) {
      ...
      double point_distance = point_2_point_VCCS_distance(epoch.cloud.row(current_point),epoch.cloud.row(labels[labels[j]]),
                          normals.row(current_point), normals.row(labels[labels[j]]), resolution);
