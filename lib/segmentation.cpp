...
{
  const auto n = data.ts1.size();
  std::vector<std::vector<double>> d(n, std::vector<double>(n));

  // Upper left corner
  d[0][0] = distance(data.ts1[0], data.ts2[0]);

  // Upper row and left-most column
  for (std::size_t i = 1; i < n; ++i) {
    d[i][0] = distance(data.ts1[i], data.ts2[0]) + d[i - 1][0];
    d[0][i] = distance(data.ts1[0], data.ts2[i]) + d[0][i - 1];
  }

  // Rest of the distance matrix
  for (std::size_t i = 1; i < n; ++i)
    for (std::size_t j = 1; j < n; ++j)
      d[i][j] = distance(data.ts1[i], data.ts2[j]) +
