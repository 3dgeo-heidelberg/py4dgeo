...
IndexType
median_calculation(
  std::vector<IndexType>& signal) // the function change the vector
{
...
local_maxima_calculation(std::vector<IndexType>& signal, IndexType order)
{
  ...
  // std::vector<double>::const_iterator right_array_left_index = signal.begin()
  ...
  while (current < signal.end()) // check main part of signal
  {
    ...
    if (current < signal.begin() + order) {
      distance_range =
        order - distance(left_array_left_index, left_array_right_index);
      if (distance_range == order)
        max_left_array_num =
          *(std::max_element((signal.end() - distance_range), signal.end()));
      else
        max_left_array_num = max(
          *max_left_array,
          *(std::max_element((signal.end() - distance_range), signal.end())));
    }
      ...
      distance_range =
        order - distance(right_array_left_index, right_array_right_index);
      if (distance_range == order)
        max_right_array_num =
          *(std::max_element(signal.begin(), signal.begin() + distance_range));
      else
        max_right_array_num = max(
          *max_right_array,
          *(std::max_element(signal.begin(), signal.begin() + distance_range)));
    }
...
IndexType
cost_L1_error(std::vector<IndexType>& signal,
              int start,
              int end,
              IndexType min_size)
{
...
std::vector<IndexType>
fit_change_point_detection(std::vector<IndexType>& signal,
                           IndexType width,
                           IndexType jump,
                           IndexType min_size)
{
...
IndexType
sum_of_costs(std::vector<IndexType>& signal,
...
std::vector<IndexType>
predict_change_point_detection(std::vector<IndexType>& signal,
                               std::vector<IndexType>& score,
                               IndexType width,
                               IndexType jump,
                               IndexType min_size,
