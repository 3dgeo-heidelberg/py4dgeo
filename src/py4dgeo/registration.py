...
def calculate_bounding_box_change(bounding_box_min,bounding_box_max, transformation_matrix):
...
def registration_method(reference_epoch, epoch, dis_threshold, lmdd, res1, res2, resolution =10, k=10, minSVPvalue=10, reduction_point=None):
    ''' 
    ...
    supervoxels_pc1 = _py4dgeo.segment_pc_in_supervoxels(reference_epoch, reference_epoch.kdtree, reference_epoch.normals, resolution, k, minSVPvalue)
    supervoxels_pc2 = _py4dgeo.segment_pc_in_supervoxels(epoch, epoch.kdtree, epoch.normals, resolution, k, minSVPvalue)
    ...
    while stage4 == 0:
        ...
        #Calculation CT2-CT1
        corCT_dist = compute_cor_distance(centroids_pc1, centroids_pc2, clouds_pc1, len(reference_epoch.cloud)) # RENAME
        #Calculation BP2-CT1
        corBP_dist = compute_cor_distance(centroids_pc1, boundary_points_pc2, clouds_pc1, len(reference_epoch.cloud)) # RENAME
        #calculation BP2- CP1
        corPC_dist = compute_cor_distance(reference_epoch, boundary_points_pc2, clouds_pc1, len(reference_epoch.cloud)) # RENAME
        ...
        for i, cloud in enumerate(centroids_pc2.cloud):
            if corCT_dist[i] < dis_threshold and all(
                corBP_dist[j + 6 * i] < dis_threshold and corPC_dist[j + 6 * i] < dt_point
                for j in range(6)
            ):
        ...
        trans_mat_cur_obj = point_to_plane_icp(
            reference_epoch, stablePC2, max_iterations=50, tolerance=0.00001, reduction_point=reduction_point
        )
        ...
        max_bb_change = calculate_bounding_box_change(initial_min_bound, initial_max_bound, trans_mat_cur)
        ...
        _py4dgeo.transform_pointcloud_inplace(epoch.cloud, transMatFinal, reduction_point)
        _py4dgeo.transform_pointcloud_inplace(centroids_pc2.cloud, transMatFinal, reduction_point)
        _py4dgeo.transform_pointcloud_inplace(boundary_points_pc2.cloud, transMatFinal, reduction_point)
        for i in range(len(clouds_pc2)):
            _py4dgeo.transform_pointcloud_inplace(clouds_pc2[i], transMatFinal, reduction_point)
