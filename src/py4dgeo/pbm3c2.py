...
def generate_extended_y_from_prior_knowledge(
    ...
    for row in pairs_of_points:
        ...
        candidates_seg_epoch0 = epoch0.radius_search(seg_epoch0, threshold_max_distance)
        candidates_seg_epoch1 = epoch1.radius_search(seg_epoch1, threshold_max_distance)
...
def add_no_corresponding_seg(
    ...
    for index, row in enumerate(epoch0_set):
        ...
        candidates_seg_epoch1 = epoch1.radius_search(row, threshold_max_distance)
...
class PerPointComputation(BaseTransformer):
    ...
    def _transform(self, X):
        ...
        for current_epoch in _epoch:
            current_epoch.validate_search_tree()
        ...
        return np.apply_along_axis(
            lambda x: self._llsv_and_pca(
                ...
                _epoch[int(x[self.columns.EPOCH_ID_COLUMN])].cloud[
                    _epoch[int(x[self.columns.EPOCH_ID_COLUMN])].radius_search(
...
class ClassifierWrapper(ClassifierMixin, BaseEstimator):
    ...
    def predict(self, X):
        ...
        self.epoch1_segments.validate_search_tree()
